...
def main():
    ...
    column = ['Role', 'Region', 'State', 'Company_type', 'Status', 'Source']

    positive_feed_back_count = 0
    negative_feed_back_count = 0
    waiting_feed_back_count = 0
    interviews_count = 0
    daily_minimum = 10 # the minimum number of application you can make in a day
    ...
    def save_file(df):
        df.to_csv('JobsEntry.csv', index=True)
    ...
    if not os.path.exists(file_path):
        with open(file_path, 'w', newline='') as file:
            ...
            writer.writerow(['Date', 'Company_name', 'Company_type','Region', 'State',
    ...
    if choice == 'Home':
        ...
        State =st.sidebar.selectbox('State *', State)
        ...
        job_entry = {'Date': entry_date, 'Company_name': company_name, 'Company_type': company_type,
                      'Region':region, 'State': State, 'Locality': locality, 
